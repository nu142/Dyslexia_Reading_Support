...
function calculateWER(reference, hypothesis) {
    reference = reference.toLowerCase().split(' ').filter(word => word);
    hypothesis = hypothesis.toLowerCase().split(' ').filter(word => word);
    const dp = Array(hypothesis.length + 1).fill().map(() => Array(reference.length + 1).fill(0));
...
// Detect letter reversals (e.g., b/d, p/q)
function detectLetterReversals(transcript, reference) {
    ...
    for (let i = 0; i < Math.min(transcriptWords.length, referenceWords.length); i++) {
...
// Detect syllable omissions (simplified)
...
// Analyze reading for dyslexia classes
function analyzeReading(spokenText, originalText, durationSec) {
    const wordCount = originalText.split(' ').length;
    const expectedDuration = wordCount * 0.5; // 2 words per second as baseline
    ...
    // Auditory: Slow reading speed indicates difficulty processing sounds
    ...
    // Phonological: High WER or syllable omissions
    ...
    // Visual: Letter reversals or high WER
    ...
    // Math: Not directly assessed in reading, so assign neutral score
    readingScores.math = 1;

    // Memory: Slow reading or high WER indicates retention issues
...
// Submit reading data and redirect
